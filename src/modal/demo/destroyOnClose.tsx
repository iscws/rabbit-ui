...
export default () => {
  ...
  return (
    <div>
      <button onClick={e => showModal()} style={{ marginRight: "15px" }}>关闭时不销毁子元素</button>
      <Modal title="不销毁子元素" open={isModalOpen} toOk={e => handleOk()} toCancel={handleCancel}>
        <div>
          <NoDestroy />
        </div>
      </Modal>
      ...
      <Modal title="销毁子元素" destroyOnClose={true} open={isModalOpen2} toOk={e => handleOk(false)} toCancel={e => handleCancel(false)}>
        <div>
          <Destroy />
        </div>
      </Modal>
