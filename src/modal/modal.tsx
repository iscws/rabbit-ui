...
const Modal: React.FC<ModalProps> = ({
      ...
      <CSSTransition classNames={'rabbit-modal'} in={animateStart} timeout={300}>
        <div
          className={classNames(
            ...
            center ? '' : 'rabbit-modal-content-default',
          )}
          style={{
            width,
            ...style,
        ...
        >
          {closable && (
            <div className="rabbit-modal-close-x" onClick={modalCancel}>
              <svg
              ...
              footer === 'default' ? (
                <>
                  <span className="rabbit-modal-footer-item">
                    <button className="rabbit-modal-button rabbit-default" onClick={modalCancel}>
                      取消
                    </button>
                  </span>

                  <span className="rabbit-modal-footer-item">
                    <button className="rabbit-modal-button rabbit-primary" onClick={modalOk}>
                      确定
                    </button>
                  </span>
                </>
                ...
                toArray(footer).map((item, index) => {
                  return (
                    <span className="rabbit-modal-footer-item" key={index}>
                      {item}
                    </span>
                  );
                })
  ...
  return outside ? createPortal(modal, document.body) : modal;
};
